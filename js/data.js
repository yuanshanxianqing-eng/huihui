...
function loadData() {
  try {
    ...
    if (raw) {
      ...
      // 旧版“基础资料 -> 网页背景”已停用。主题设置是唯一的全局背景来源。
      // 这里只清掉旧字段，避免旧数据在后续 saveData() 时继续占空间或重新参与背景逻辑。
      if (parsed.settings && Object.prototype.hasOwnProperty.call(parsed.settings, 'bgImage')) {
        delete parsed.settings.bgImage;
      }
