...
function applyCustomTheme() {
  ...
  // 页面背景只由 themeSettings 管理。不要再与基础资料背景或额外 localStorage 副本竞争。
  if (t.pageBgImage) {
    body.classList.add('custom-theme-bg-image', 'has-bg');
...
async function themeCompressBackground(file) {
  // 背景图仍保存在主数据里，但先压缩，避免一张手机原图直接把 localStorage 塞满。
  if (typeof patchCompressImageFile === 'function') {
    return patchCompressImageFile(file, {
      maxWidth: 1440,
      maxHeight: 1440,
      quality: 0.76,
...
async function handleThemeBgUpload(e) {
  ...
  try {
    const dataUrl = await themeCompressBackground(file);
    appData.themeSettings.pageBgImage = dataUrl;
    appData.themeSettings.enabled = true;

    // 默认 100% 的颜色遮罩会完全盖住图片。首次导入时自动调低，之后尊重用户自己的设置。
    if (!oldImage && (!Number.isFinite(oldOpacity) || oldOpacity > 0.85)) {
      appData.themeSettings.pageBgOpacity = 0.18;
    }

    // 只保存一份：appData.themeSettings.pageBgImage。保存失败则恢复旧图，避免“出现一下又消失”。
    saveData();
    applyCustomTheme();
    renderThemeCustomizePanel();
  } catch (err) {
    ...
    alert('背景图片保存失败：' + (err && err.message ? err.message : '未知错误'));
  } finally {
