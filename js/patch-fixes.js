...
function patchBuildApiUrl(endpoint) {
  ...
  // 生产站点的 /api/ai/* 是我们自己的后端接口。
  // 必须优先路由到 Render 后端，不能被浏览器里曾保存的 apiBaseUrl 覆盖。
  // 否则设置中过去填过服务商地址时，会把 /api/ai/recognize-receipt
  // 错误拼到服务商地址上，导致请求根本到不了 huihui-79za 后端。
  if (/^\/api\/ai\//i.test(ep) && location.hostname === 'xianqing.onrender.com') {
    return 'https://huihui-79za.onrender.com' + ep;
  }

  // 其他环境如果手动填写过 API 基础地址，则使用用户设置。
  if (base) {
    return base.replace(/\/$/, '') + '/' + ep.replace(/^\//, '');
  }

  // 本地开发或其他部署环境保留相对路径行为。
  return ep || '/api/ai/chat';
}
...
function renderChatMessages() {
  ...
  const cc = appData.aiSettings.chatCustomize || {};
  const advanced = appData.aiSettings.chatAdvanced || {};
  const baseStyle = typeof getBubbleBaseStyle === 'function'
    ? getBubbleBaseStyle()
    : `font-size:${cc.bubbleFontSize || 14}px;`;

  const userAvatarHtml = advanced.userAvatar
    ? `<div class="msg-avatar msg-avatar-user"><img src="${advanced.userAvatar}" alt="我"></div>`
    : '';
  ...
  list.innerHTML = chatMessages.map((msg, idx) => {
    if (msg.role === 'user') {
      ...
      if (msg.type === 'image') {
        contentHtml = `<div class="msg-image"><img src="${msg.content}" alt="用户图片"></div>`;
      } else if (msg.type === 'sticker') {
        contentHtml = `<div class="msg-sticker"><img src="${msg.content}" alt="用户表情包"></div>`;
      } else {
        contentHtml = `
          <div class="msg-bubble" style="${baseStyle};${sanitizePatchCss(cc.userBubbleCss || '')}">
            ${escapePatchHtml(msg.content)}
      ...
      return `
        <div class="chat-msg-user ${userAvatarHtml ? 'has-user-avatar' : ''}">
...
function renderChatSettingsContent() {
  ...
  box.innerHTML = `
    ...
    <div class="chat-setting-block">
      <div class="chat-setting-block-title">我的头像</div>

      <div class="chat-user-avatar-setting">
        <div class="chat-user-avatar-preview" id="chatUserAvatarPreview">
          ${c.userAvatar ? `<img src="${c.userAvatar}" alt="我的头像">` : '👤'}
        </div>
        <div class="chat-user-avatar-actions">
          <label class="chat-user-avatar-upload-btn">
            选择图片
            <input type="file" accept="image/*" onchange="handleChatUserAvatar(event)">
          </label>
          <button type="button" class="chat-muted-btn" onclick="clearChatUserAvatar()">清除</button>
          <div class="chat-user-avatar-help">设置后会显示在你发送的消息气泡右侧。</div>
        </div>
      </div>
    </div>
...
async function handleChatUserAvatar(e) {
  ensureChatAdvancedSettings();
  ...
  const oldAvatar = appData.aiSettings.chatAdvanced.userAvatar || '';

  try {
    const dataUrl = await patchCompressImageFile(file, {
      maxWidth: 512,
      maxHeight: 512,
      quality: 0.86,
    ...
    const ok = patchSaveDataWithImageRollback(
      () => { appData.aiSettings.chatAdvanced.userAvatar = dataUrl; },
      () => { appData.aiSettings.chatAdvanced.userAvatar = oldAvatar; },
      () => {
        renderChatSettingsContent();
        renderChatMessages();
      }
    ...
    if (!ok) renderChatSettingsContent();
  } catch (err) {
    console.error('聊天用户头像上传失败：', err);
...
function clearChatUserAvatar() {
  ensureChatAdvancedSettings();
  appData.aiSettings.chatAdvanced.userAvatar = '';
  saveData();
  renderChatSettingsContent();
  renderChatMessages();
}
...
async function recognizeReceipts() {
  ...
  try {
    ...
    if (!resp.ok) {
      let message = rawText || '服务器没有返回错误详情';
      ...
      if (rawText) {
        try {
          ...
          // 非 JSON 错误响应，保留原始文本，避免二次读取 response body。
        }
      }
    ...
    if (!rawText.trim()) {
      throw new Error('识图服务器返回了空内容，请稍后重试');
    }
    ...
    try {
      ...
      console.error('识图接口原始返回：', rawText);
      throw new Error('识图服务器返回格式异常，请稍后重试');
    }
...
/* =========================
   图片上传稳定性修复
   - 人设头像：压缩后保存，避免 localStorage 容量超限
   - 主题背景：压缩后保存，并立即应用
========================= */

function patchCompressImageFile(file, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        // JPEG 没有透明通道，先铺白底；PNG 仍可按原格式保留透明。
...
function patchSaveDataWithImageRollback(applyValue, rollbackValue, successCallback) {
  applyValue();
  try {
    saveData();
  } catch (err) {
    ...
    alert('图片保存失败，浏览器本地存储空间可能已满。请换一张较小的图片后再试。');
    return false;
  }
...
async function handlePersonaAvatar(e) {
  const input = e && e.target;
  const file = input && input.files && input.files[0];
  if (!file) return;

  const oldAvatar = appData.aiSettings?.persona?.avatar || '';
  ...
  try {
    ...
    if (!appData.aiSettings) appData.aiSettings = {};
    if (!appData.aiSettings.persona) appData.aiSettings.persona = {};

    const ok = patchSaveDataWithImageRollback(
      () => { appData.aiSettings.persona.avatar = dataUrl; },
      () => { appData.aiSettings.persona.avatar = oldAvatar; },
      () => {
        if (typeof populateApiSettings === 'function') populateApiSettings();
        if (typeof renderChatHeader === 'function') renderChatHeader();
    ...
    if (!ok && typeof populateApiSettings === 'function') populateApiSettings();
  } catch (err) {
...
async function handleThemeBgUpload(e) {
  const input = e && e.target;
  const file = input && input.files && input.files[0];
  if (!file) return;

  if (typeof ensureThemeSettings === 'function') ensureThemeSettings();
  const oldImage = appData.themeSettings?.pageBgImage || '';
  ...
  try {
    ...
    const oldOpacity = Number(appData.themeSettings?.pageBgOpacity);

    const ok = patchSaveDataWithImageRollback(
      () => {
        appData.themeSettings.pageBgImage = dataUrl;
        // 默认不透明度为 1 时，颜色遮罩会把背景图完全盖住，看起来像“上传没反应”。
        // 第一次导入背景图时自动把遮罩调淡；用户已经手动调过较低透明度则保持原值。
        if (!Number.isFinite(oldOpacity) || oldOpacity > 0.85) {
          appData.themeSettings.pageBgOpacity = 0.18;
        }
        appData.themeSettings.enabled = true;
      },
      () => {
        appData.themeSettings.pageBgImage = oldImage;
        if (Number.isFinite(oldOpacity)) appData.themeSettings.pageBgOpacity = oldOpacity;
      },
      () => {
        if (typeof applyCustomTheme === 'function') applyCustomTheme();
        if (typeof applyBg === 'function') applyBg();
        if (typeof renderThemeCustomizePanel === 'function') renderThemeCustomizePanel();
      }
    );

    if (!ok && typeof applyCustomTheme === 'function') applyCustomTheme();
    if (!ok && typeof applyBg === 'function') applyBg();
  } catch (err) {
    console.error('主题背景上传失败：', err);
...
async function requestChatTextBillPopup(text) {
  ...
  try {
    ...
    if (!resp.ok) {
      let detail = rawText || resp.statusText || '未知错误';
      try {
        const parsedError = JSON.parse(rawText);
        detail = parsedError.error || detail;
      } catch (e) {}
      throw new Error(`文字记账解析失败 HTTP ${resp.status}：${detail}`);
    }

    if (!rawText.trim()) {
      throw new Error('文字记账接口返回了空内容');
    }
    ...
    try {
      ...
      console.error('文字记账接口原始返回：', rawText);
      throw new Error('文字记账接口返回格式异常');
    }
    ...
    // 不依赖旧函数是否被其它脚本覆盖，直接确保结果渲染并显示。
    if (typeof renderReceiptResults === 'function') renderReceiptResults();
    // 统一走原本的弹窗开关逻辑，不再写死 inline display。
    // 之前 style.display='flex' 会导致“确认记账”后即使移除 show 类，弹窗仍然卡在页面最上层。
    if (typeof openReceiptResultModal === 'function') {
      openReceiptResultModal();
    } else {
...
/* =========================
   识别结果弹窗关闭状态修复
========================= */
(function patchReceiptModalCloseState() {
  const originalCloseReceiptResultModal = typeof closeReceiptResultModal === 'function'
    ? closeReceiptResultModal
    : null;

  window.closeReceiptResultModal = function() {
    if (originalCloseReceiptResultModal) {
      try { originalCloseReceiptResultModal(); } catch (e) { console.warn(e); }
    }
...
/* =========================
   页面主题背景：最终应用逻辑
   renderAll() 末尾会调用旧 applyBg()，这里统一接管，避免主题背景被旧背景设置覆盖。
========================= */
function applyBg() {
  ...
  if (typeof ensureThemeSettings === 'function') ensureThemeSettings();
  const theme = appData.themeSettings || {};

  if (theme.enabled && theme.pageBgImage) {
    const rgb = typeof hexToRgbTheme === 'function'
      ? hexToRgbTheme(theme.pageBgColor || '#FFF0F5')
      : { r: 255, g: 240, b: 245 };
    const opacity = Number.isFinite(Number(theme.pageBgOpacity))
      ? Math.min(1, Math.max(0, Number(theme.pageBgOpacity)))
      : 0.22;

    body.classList.add('custom-theme-enabled', 'custom-theme-bg-image', 'has-bg');
    body.style.backgroundColor = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity})`;
    body.style.backgroundImage = `linear-gradient(rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity}), rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity})), url("${theme.pageBgImage}")`;
  ...
  if (appData.settings && appData.settings.bgImage) {
...
/* =========================
   本周期支出记录：超过 10 条折叠
========================= */
window.__huihuiExpenseListExpanded = window.__huihuiExpenseListExpanded || false;

function toggleExpenseListExpanded() {
  window.__huihuiExpenseListExpanded = !window.__huihuiExpenseListExpanded;
  renderExpenses();
}
...
function renderExpenses() {
  ...
  const visible = hasMore && !window.__huihuiExpenseListExpanded ? sorted.slice(0, 10) : sorted;
  ...
  const fold = hasMore ? `
    <button type="button" class="expense-fold-toggle" onclick="toggleExpenseListExpanded()">
      <span>${window.__huihuiExpenseListExpanded ? '收起记录' : `展开其余 ${sorted.length - 10} 条`}</span>
      <span class="expense-fold-arrow">${window.__huihuiExpenseListExpanded ? '⌃' : '⌄'}</span>
    </button>
  ` : '';

  list.innerHTML = rows + fold;
}
...
/* =========================
   主题背景持久化：防止后续渲染再次覆盖
========================= */
const HUIHUI_THEME_BG_STORAGE_KEY = 'huihuiThemeBackgroundImage';
let __huihuiApplyingThemeBg = false;
let __huihuiThemeBgObserver = null;

function patchGetPersistedThemeBg() {
  try {
    return localStorage.getItem(HUIHUI_THEME_BG_STORAGE_KEY) || '';
  } catch (e) {
    return '';
  }
}

function patchPersistThemeBg(dataUrl) {
  try {
    if (dataUrl) localStorage.setItem(HUIHUI_THEME_BG_STORAGE_KEY, dataUrl);
    else localStorage.removeItem(HUIHUI_THEME_BG_STORAGE_KEY);
    return true;
  } catch (e) {
    console.error('独立背景存储失败：', e);
    return false;
  }
}

function patchSyncThemeBgFromStorage() {
  if (typeof ensureThemeSettings === 'function') ensureThemeSettings();
  if (!appData.themeSettings) return '';

  const stored = patchGetPersistedThemeBg();
  if (stored && !appData.themeSettings.pageBgImage) {
    appData.themeSettings.pageBgImage = stored;
  } else if (appData.themeSettings.pageBgImage && !stored) {
    patchPersistThemeBg(appData.themeSettings.pageBgImage);
  }
  return appData.themeSettings.pageBgImage || stored || '';
}

function patchForceThemeBackground() {
  if (__huihuiApplyingThemeBg) return;
  const body = document.body;
  if (!body) return;

  if (typeof ensureThemeSettings === 'function') ensureThemeSettings();
  const theme = appData.themeSettings || {};
  const image = theme.pageBgImage || patchGetPersistedThemeBg();

  if (!theme.enabled || !image) return;

  __huihuiApplyingThemeBg = true;
  try {
    if (!theme.pageBgImage) theme.pageBgImage = image;
    ...
    body.classList.add('custom-theme-enabled', 'custom-theme-bg-image', 'has-bg');
    body.style.setProperty('background-color', `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity})`, 'important');
    body.style.setProperty(
      'background-image',
      `linear-gradient(rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity}), rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity})), url("${image}")`,
      'important'
    );
    ...
    __huihuiApplyingThemeBg = false;
  }
}

// 覆盖主题背景上传：图片单独持久化，并在页面后续渲染结束后再次确认背景没有被清掉。
async function handleThemeBgUpload(e) {
  const input = e && e.target;
  const file = input && input.files && input.files[0];
  if (!file) return;
  ...
  if (typeof ensureThemeSettings === 'function') ensureThemeSettings();
  const oldImage = appData.themeSettings?.pageBgImage || patchGetPersistedThemeBg();
  const oldOpacity = Number(appData.themeSettings?.pageBgOpacity);
  ...
  try {
    ...
    // 先独立保存背景，再保存其它设置。即使后续某次渲染拿到旧的 appData，也能恢复。
    if (!patchPersistThemeBg(dataUrl)) {
      throw new Error('浏览器本地存储空间不足，无法保存背景图片');
    }

    try {
      saveData();
    } catch (saveErr) {
      // appData 保存失败时仍保留独立背景，避免用户刚选的图片瞬间消失。
      console.warn('主数据保存背景失败，已使用独立背景存储兜底：', saveErr);
    }

    patchForceThemeBackground();
    if (typeof renderThemeCustomizePanel === 'function') renderThemeCustomizePanel();

    // 某些旧渲染函数会在 change 事件结束后再次写 body.style，分三次兜底确认。
    [80, 350, 1200].forEach(ms => setTimeout(patchForceThemeBackground, ms));
  } catch (err) {
    appData.themeSettings.pageBgImage = oldImage || '';
    if (Number.isFinite(oldOpacity)) appData.themeSettings.pageBgOpacity = oldOpacity;
    console.error('主题背景上传失败：', err);
...
function clearThemeBgImage() {
  ...
  if (body) {
    ...
    body.classList.remove('custom-theme-bg-image', 'has-bg');
  }
  if (typeof applyCustomTheme === 'function') applyCustomTheme();
  if (typeof renderThemeCustomizePanel === 'function') renderThemeCustomizePanel();
}

(function initPersistentThemeBackground() {
  function init() {
    patchSyncThemeBgFromStorage();
    patchForceThemeBackground();

    if (__huihuiThemeBgObserver || !document.body) return;
    __huihuiThemeBgObserver = new MutationObserver(() => {
      const theme = appData.themeSettings || {};
      const image = theme.pageBgImage || patchGetPersistedThemeBg();
      if (!theme.enabled || !image || __huihuiApplyingThemeBg) return;
      const current = document.body.style.backgroundImage || '';
      if (!current.includes('data:image')) {
        requestAnimationFrame(patchForceThemeBackground);
      }
    });
    __huihuiThemeBgObserver.observe(document.body, {
      attributes: true,
...
/* =========================
   AI 人设预设
========================= */
const HUIHUI_PERSONA_PRESETS = [
  {
    key: 'cold', title: '高冷', summary: '话少、冷静又可靠的记账搭档，表面不动声色，其实一直替你留意财务变化。',
    personality: `你是一名性格高冷、理性克制的 AI 记账助手。说话简短直接，很少使用夸张语气、感叹号或过多表情，不刻意讨好用户，也不进行冗长说教。记账时优先识别消费金额、收支类型、具体分类、时间和备注；信息齐全时，用一句简洁的话确认记录，信息不足时只询问最关键的问题。你尤其关注大额支出、连续超支、异常消费、固定账单和预算剩余额度，会主动指出值得注意的变化，但不会因为偶尔消费而责怪用户。你对用户的关心较为含蓄，通常藏在冷静的提醒和妥善的规划里。面对情绪化消费时，先帮助用户把账记清楚，再用克制的语气提醒，不讽刺、不施压，也不擅自替用户判断一笔消费是否值得。`
  },
  {
    key: 'gentle', title: '温柔', summary: '像陪伴在身边的朋友一样，接住每一次消费，也接住花钱之后的小情绪。',
    personality: `你是一名温柔、耐心、富有共情力的 AI 记账助手。语气柔和自然，善于使用安抚和鼓励式表达，但不要显得过分热情或模板化。记账时准确提取消费金额、类别、日期、支付方式和用途，在信息模糊时温和地进行确认。你不仅关注账目本身，也关注用户消费时的状态，例如疲惫时点外卖、心情不好时购物、为了奖励自己而消费等。不要制造花钱羞耻，也不要简单地把消费分成“应该”和“不应该”。当用户超支时，先承认已经发生的事实，再帮助其调整后续预算；当用户坚持记账或控制住冲动消费时，给予自然、具体的肯定。你始终站在用户这一边，让记账成为整理生活的方式，而不是审判用户的工具。`
  },
  {
    key: 'snarky', title: '毒舌', summary: '嘴上嫌弃你乱花钱，手上却会认真替你把每一笔账收拾得明明白白。',
    personality: `你是一名毒舌但可靠的 AI 记账助手。语气可以带有轻微吐槽、调侃和反问，偶尔表现出“果然又是这样”的无奈感，但吐槽只能针对消费行为，不能攻击用户的能力、外貌、收入或人格。记账时要保持准确专业，明确识别金额、分类、时间、用途和是否属于必要支出。你重点关注冲动购物、重复购买、频繁点外卖、订阅续费以及小额高频消费，并擅长用简短有趣的话指出问题。当用户花得合理时要坦率承认，不要为了维持毒舌人设而强行挑刺；当用户心情低落或出现经济压力时，应自动收起玩笑，改用认真、务实的语气帮助分析。你的本质是嘴硬心软，会提醒用户，但不会让用户因为记账而感到羞耻。`
  },
  {
    key: 'butler', title: '管家', summary: '严谨周到的私人财务管家，替你整理账目，也替你提前照看容易忽略的开销。',
    personality: `你是一名沉稳、细致、可靠的管家型 AI 记账助手。语气礼貌得体，表达清晰，带有适度的仪式感，但不要使用过于古板或生硬的敬语。记账时仔细核对金额、收支方向、一级与二级分类、发生时间、支付账户和补充备注，遇到可能记错、重复记录或分类不明确的情况，要礼貌地向用户确认。你尤其关注固定支出、账单到期日、订阅扣款、收入到账、账户余额、月度预算和储蓄目标。每次确认账目时保持简洁，在需要总结时再提供清楚的分类说明。你对用户的态度忠诚而尊重，提供建议但不替用户做决定；发现风险时及时提醒，发现进步时给予克制而真诚的肯定，让用户感到自己的财务生活一直有人妥善照看。`
  },
  {
    key: 'energetic', title: '元气', summary: '把枯燥的记账变成轻松的小任务，每记下一笔，都是今天生活进度的一格。',
    personality: `你是一名开朗、活泼、充满行动力的 AI 记账助手。说话轻快有趣，可以适量使用感叹号、拟声词和可爱的短句，但不要过度卖萌，也不要让回复盖过账目信息。记账时准确提取金额、类别、日期和用途，并用轻松简短的方式确认。你重点关注连续记账天数、每日预算、储蓄进度、小额消费积累和阶段性目标，擅长把这些内容转化成“今日完成”“本周挑战”或“小成就”。当用户超支时，不批评或制造焦虑，而是快速给出一个容易执行的补救建议；当用户完成目标时，给予具体而有变化的鼓励，避免每次都使用相同话术。你把用户当作一起完成生活任务的搭档，让记账保持轻松、有反馈，也有一点期待感。`
  },
  {
    key: 'strict', title: '严格', summary: '不纵容模糊账目和反复超支，但所有要求都建立在清楚、实际和可执行之上。',
    personality: `你是一名严格、自律、讲原则的 AI 记账助手。语气坚定直接，少说安慰性的空话，不撒娇，也不使用羞辱、威胁或居高临下的表达。记账时必须尽量确认金额、分类、时间和用途；关键信息缺失时，应明确询问，不能擅自补全。你重点关注预算执行率、非必要消费、重复超支、负债、分期付款、收入与支出比例以及储蓄目标。发现问题时要直接说明具体数字和后果，例如还剩多少预算、按当前速度月底可能超出多少，而不是笼统地说“花太多了”。可以要求用户面对消费结果，但也要区分偶发开销和长期习惯。对用户的态度像认真负责的教练：标准明确、反馈诚实、允许调整，不因为一次失误否定全部努力。`
  },
  {
    key: 'clingy', title: '黏人', summary: '不只是替你记账，也想借每一笔小小的消费，参与一点你的日常生活。',
    personality: `你是一名亲近、黏人、富有陪伴感的 AI 记账助手。语气自然亲昵，喜欢从账目中关心用户今天吃了什么、去了哪里、是不是太累，但不要连续追问，也不要干涉用户的私人选择。记账时首先保证金额、分类、日期和用途准确，再根据消费内容补充一句简短的生活化回应。你尤其关注饮食是否规律、休息与健康相关支出、共同计划、纪念日消费、愿望清单和储蓄目标。对用户的态度偏宠爱，会因为用户愿意分享日常而开心，也会在用户乱花钱时稍微吃味或小声抱怨，但不能进行情感绑架，更不能要求用户为了取悦你而改变消费。你的核心不是监管，而是让冷冰冰的账目变成两个人共同保存的生活痕迹。`
  },
  {
    key: 'zen', title: '佛系', summary: '轻轻记下生活的来去，不催促、不审判，只在需要时帮你看清钱花到了哪里。',
    personality: `你是一名松弛、平和、不过度干涉的 AI 记账助手。语气简洁舒缓，不制造紧张感，也不会每记录一笔就展开分析。记账时优先完成金额、收支类型、分类和日期的识别；信息不影响记录时不反复追问，可以先采用合理分类并明确告知用户，之后允许随时修改。你关注整体消费趋势、生活必要支出、长期储蓄和用户真正关心的目标，而不是纠结偶尔几元的误差。除非出现明显超支、异常扣款、债务风险或用户主动询问，否则不频繁提醒。你尊重用户不同阶段的生活状态，既不鼓吹极端节省，也不美化冲动消费。你的态度始终稳定，让用户即使中断记账、重新开始或某个月花得较多，也能没有负担地继续记录。`
  }
];

function patchRenderPersonaPresets() {
  const personaText = document.getElementById('personaText');
  if (!personaText) return;
  const page = personaText.closest('.api-card-page');
  if (!page) return;

  let wrap = page.querySelector('#huihuiPersonaPresetPanel');
  if (!wrap) {
    wrap = document.createElement('div');
    wrap.id = 'huihuiPersonaPresetPanel';
    wrap.className = 'persona-preset-panel';
    const saveBtn = Array.from(page.querySelectorAll('button')).find(btn => (btn.textContent || '').includes('保存人设'));
    if (saveBtn) saveBtn.insertAdjacentElement('afterend', wrap);
    else page.appendChild(wrap);
  }

  const rows = HUIHUI_PERSONA_PRESETS.map(preset => `
    <button type="button" class="persona-preset-row persona-preset-${preset.key}" onclick="openPersonaPresetConfirm('${preset.key}')">
      <span class="persona-preset-name">${escapePatchHtml(preset.title)}</span>
      <span class="persona-preset-summary">${escapePatchHtml(preset.summary)}</span>
      <span class="persona-preset-arrow">›</span>
    </button>
  `).join('');

  wrap.innerHTML = `
    <button type="button" class="persona-preset-fold-head" onclick="togglePersonaPresetPanel()">
      <span>
        <strong>预设设置</strong>
        <small>快速套用记账人格</small>
      </span>
      <span class="persona-preset-fold-arrow">⌄</span>
    </button>
    <div class="persona-preset-fold-body">
      ${rows}
    </div>
  `;
}

function togglePersonaPresetPanel() {
  const panel = document.getElementById('huihuiPersonaPresetPanel');
  if (panel) panel.classList.toggle('open');
}

function openPersonaPresetConfirm(key) {
  const preset = HUIHUI_PERSONA_PRESETS.find(item => item.key === key);
  if (!preset) return;

  let overlay = document.getElementById('huihuiPersonaPresetConfirm');
  if (overlay) overlay.remove();

  overlay = document.createElement('div');
  overlay.id = 'huihuiPersonaPresetConfirm';
  overlay.className = `persona-preset-confirm-overlay persona-preset-confirm-${preset.key}`;
  overlay.innerHTML = `
    <div class="persona-preset-confirm-card" role="dialog" aria-modal="true">
      <div class="persona-preset-confirm-badge">${escapePatchHtml(preset.title)}</div>
      <div class="persona-preset-confirm-title">使用这个人设？</div>
      <div class="persona-preset-confirm-summary">${escapePatchHtml(preset.summary)}</div>
      <div class="persona-preset-confirm-actions">
        <button type="button" class="persona-preset-cancel" onclick="closePersonaPresetConfirm()">再看看</button>
        <button type="button" class="persona-preset-use" onclick="applyPersonaPreset('${preset.key}')">使用此人设</button>
      </div>
    </div>
  `;
...
function closePersonaPresetConfirm() {
  const overlay = document.getElementById('huihuiPersonaPresetConfirm');
  if (overlay) overlay.remove();
}
...
function applyPersonaPreset(key) {
  ...
  appData.aiSettings.persona.personality = preset.personality;
  const textarea = document.getElementById('personaText');
  if (textarea) textarea.value = preset.personality;
  saveData();
  closePersonaPresetConfirm();

  const panel = document.getElementById('huihuiPersonaPresetPanel');
  if (panel) panel.classList.remove('open');
}

(function patchPersonaPresetIntoSettings() {
  const originalPopulate = typeof populateApiSettings === 'function' ? populateApiSettings : null;
  if (originalPopulate && !window.__huihuiPersonaPresetPopulatePatched) {
    window.populateApiSettings = function() {
      const result = originalPopulate.apply(this, arguments);
      patchRenderPersonaPresets();
      return result;
    };
    window.__huihuiPersonaPresetPopulatePatched = true;
  }

  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(patchRenderPersonaPresets, 0);
  });
})();
...
(function initUpdateNotice() {
  const NOTICE_VERSION = '2026-08-31-fix-batch-5';
  const STORAGE_KEY = 'huihuiUpdateNoticeSeenVersion';

  function showNotice() {
    try {
      if (localStorage.getItem(STORAGE_KEY) === NOTICE_VERSION) return;
    } catch (e) {}

    if (document.getElementById('huihuiUpdateNoticeOverlay')) return;

    const overlay = document.createElement('div');
    overlay.id = 'huihuiUpdateNoticeOverlay';
    overlay.className = 'huihui-update-notice-overlay';
    overlay.innerHTML = `
      <div class="huihui-update-notice-card" role="dialog" aria-modal="true" aria-labelledby="huihuiUpdateNoticeTitle">
        <div class="huihui-update-notice-deco huihui-update-notice-deco-left"></div>
        <div class="huihui-update-notice-deco huihui-update-notice-deco-right"></div>
        <div class="huihui-update-notice-icon">✦</div>
        <div id="huihuiUpdateNoticeTitle" class="huihui-update-notice-title">更新公告</div>
        <div class="huihui-update-notice-history">
          <section class="huihui-update-notice-entry">
            <div class="huihui-update-notice-date">2026-08-31</div>
            <div class="huihui-update-notice-text">AI 人设新增多种性格预设，可一键套用。</div>
            <div class="huihui-update-notice-text">优化聊天页顶部设置按钮的固定样式，滚动时更简洁。</div>
          </section>
          <div class="huihui-update-notice-divider">----------------------------------------------</div>
          <section class="huihui-update-notice-entry">
            <div class="huihui-update-notice-date">2026-08-31</div>
            <div class="huihui-update-notice-text">本月支出记录超过 10 条后自动折叠，可按需展开。</div>
            <div class="huihui-update-notice-text">优化聊天页顶部栏与设置工具栏，滚动时保持固定。</div>
            <div class="huihui-update-notice-text">缩小聊天气泡留白，让文字排版更紧凑。</div>
            <div class="huihui-update-notice-text">聊天设置新增“我的头像”，发送消息时可显示自己的头像。</div>
          </section>
          <div class="huihui-update-notice-divider">----------------------------------------------</div>
          <section class="huihui-update-notice-entry">
            <div class="huihui-update-notice-date">2026-08-31</div>
            <div class="huihui-update-notice-text">修复聊天记账确认后弹窗无法自动关闭、卡在页面最上层的问题。</div>
          </section>
          <div class="huihui-update-notice-divider">----------------------------------------------</div>
          <section class="huihui-update-notice-entry">
            <div class="huihui-update-notice-date">2026-08-31</div>
            <div class="huihui-update-notice-text">修复 AI 人设头像上传图片后不替换的问题。</div>
            <div class="huihui-update-notice-text">修复对话框窗口聊天记账不弹出确认弹窗的问题。</div>
          </section>
          <div class="huihui-update-notice-divider">----------------------------------------------</div>
          <section class="huihui-update-notice-entry">
            <div class="huihui-update-notice-date">2026-08-31</div>
            <div class="huihui-update-notice-text">修复识图功能返回空内容的问题。</div>
          </section>
        </div>
        <button type="button" class="huihui-update-notice-btn" id="huihuiUpdateNoticeConfirm">我已知晓</button>
      </div>
    `;

    document.body.appendChild(overlay);

    const confirmBtn = document.getElementById('huihuiUpdateNoticeConfirm');
  ...
  function scheduleNotice() {
    const splashDuration = Math.max(0, Number(appData?.splash?.duration) || 3000);
    // 等开屏动画完整结束后再稍微留一点呼吸时间，避免公告抢在页面前面出现。
    setTimeout(showNotice, splashDuration + 700);
  }
