...
/* =========================
   全局背景数据迁移：只保留主题设置这一套
========================= */
(function migrateLegacyPageBackground() {
  function migrate() {
    if (typeof ensureThemeSettings === 'function') ensureThemeSettings();
    if (!appData.themeSettings) return;

    let changed = false;
    let legacyStandalone = '';
    try {
      legacyStandalone = localStorage.getItem('huihuiThemeBackgroundImage') || '';
    } catch (e) {}

    const legacyBasic = appData.settings && appData.settings.bgImage ? appData.settings.bgImage : '';

    // 优先保留主题设置里现有图片；若旧补丁曾只存进独立副本，则迁回正式主题字段。
    if (!appData.themeSettings.pageBgImage && legacyStandalone) {
      appData.themeSettings.pageBgImage = legacyStandalone;
      changed = true;
    } else if (!appData.themeSettings.pageBgImage && legacyBasic) {
      appData.themeSettings.pageBgImage = legacyBasic;
      changed = true;
    }

    if (appData.settings && Object.prototype.hasOwnProperty.call(appData.settings, 'bgImage')) {
      delete appData.settings.bgImage;
      changed = true;
    }

    try {
      if (localStorage.getItem('huihuiThemeBackgroundImage') !== null) {
        localStorage.removeItem('huihuiThemeBackgroundImage');
      }
    } catch (e) {}

    if (changed) {
      try { saveData(); } catch (e) { console.warn('背景数据迁移保存失败：', e); }
    }
...
(function initUpdateNotice() {
  const NOTICE_VERSION = '2026-08-31-bg-unified-1';
      ...
      <div class="huihui-update-notice-card" role="dialog" aria-modal="true" aria-labelledby="huihuiUpdateNoticeTitle">
        ...
        <div class="huihui-update-notice-history">
          <section class="huihui-update-notice-entry">
            ...
            <div class="huihui-update-notice-text">统一网页背景设置，现集中由“主题设置”管理。</div>
            <div class="huihui-update-notice-text">移除基础资料中的旧网页背景入口，避免两套背景设置互相覆盖。</div>
          </section>
